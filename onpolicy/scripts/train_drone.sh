...
num_agents=5
algo="rmappo"
exp="5agents_speed2_random_1.5_fix_obs_reward_more_info_mini5_con"
seed_max=1
...
do
    echo "seed is ${seed}:"
    CUDA_VISIBLE_DEVICES=1 python train/train_drone.py --env_name ${env} --algorithm_name ${algo} \
    --experiment_name ${exp} --scenario_name ${scenario} --num_agents ${num_agents} \
    --seed ${seed} --n_training_threads 1 --n_rollout_threads 1024 \
    --exe_num_mini_batch 5 --ctl_num_mini_batch 1 --episode_length 121 --num_env_steps 2000000000 \
    ...
    --model_dir "./results/Drone/Nav_Drone/rmappo/5agents_speed2_random_1.5_fix_obs_reward_more_info_mini5/wandb/run-20221011_084144-2iptcs4s/files"
done
