...
class DroneHRunner(HRunner):
    """Runner class to perform training, evaluation. and data collection for the MPEs. See parent class for details."""
    def __init__(self, config):
        super(DroneHRunner, self).__init__(config)
    ...
    def init_exe_input(self):
        self.exe_input= {}
        self.exe_input['agent_state'] = np.zeros((self.n_rollout_threads, self.num_agents, 1, 8))
        self.exe_input['target_goal'] = np.zeros((self.n_rollout_threads, self.num_agents, 1, 2))
        self.exe_input['other_pos'] = np.zeros((self.n_rollout_threads, self.num_agents, self.num_agents-1, 8))
